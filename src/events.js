...
player.on('trackAdd', (queue, track) => {
    queue.metadata.send(`Le son " ${track.title} " a été ajouté dans la file d'attente  ✅`);
});

player.on('botDisconnect', (queue) => {
    queue.metadata.send("J'ai été déconnecté manuellement du canal vocal, en vidant la file dattente ❌");
});

player.on('channelEmpty', (queue) => {
    queue.metadata.send("Personne n'est dans le canal vocal ! ❌");
});
