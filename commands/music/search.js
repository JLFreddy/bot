...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        embed.setDescription(`${maxTracks.map((track, i) => `**${i + 1}**. ${track.title} | ${track.author}`).join('\n')}\n\nSélectionnez le choix entre **1** et **${maxTracks.length}** ou **Annuler** ⬇️`);
        ...
        collector.on('collect', async (query) => {
            if (query.content.toLowerCase() === 'Annuler') return message.channel.send(`Recherche annulée ✅`) && collector.stop();

            const value = parseInt(query.content);

            if (!value || value <= 0 || value > maxTracks.length) return message.channel.send(`Réponse non valide, essayez une valeur comprise entre **1** et **${maxTracks.length}** or **Annuler** ❌`);
        ...
        collector.on('end', (msg, reason) => {
            if (reason === 'time') return message.channel.send(`La recherche a expiré ${message.author} ❌`);
        });
    },
};